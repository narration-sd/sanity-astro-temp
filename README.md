...
- if the issues show up, the app not functioning correctly, then simply remove _all_ node_modules and the pnpm-lock.yaml file, then run pnpm install. Don't forget afterwards to re-add pnpm-lock to your git so it'll always be included.
...
1. using the projectId, dataset, and Viewer Token from the sanity.io project, create your own .env.development, based on the .env_example.
...
- More to come here, but the first is to use the .env_examplw to assure you satisfy the needs. This will update along with SSR-SSG switching as that gets added soon.